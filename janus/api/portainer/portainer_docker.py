...
from janus.api.models import Node, Network, SessionRequest, AddEndpointRequest
...
def auth(func):
    def wrapper(self, *args, **kwargs):
        def authenticate():
            ...
            res = aa_api.authenticate_user(
                AuthenticateUserRequest(pcfg.username, pcfg.password)
            )
...
class PortainerDockerApi(Service):
    ...
    def _get_docker_info(self, nid: int, **kwargs):
        ...
        res = self._call("/endpoints/{}/docker/info".format(nid), "GET", None, **kwargs)
    ...
    def get_images(self, node: Node, **kwargs):
        kwargs["_return_http_data_only"] = True
        res = self._call(
            "/endpoints/{}/docker/images/json".format(node.id), "GET", None, **kwargs
        )
        string = res.read().decode("utf-8")
        return json.loads(string)
    ...
    def pull_image(self, node: Node, img, tag):
        ...
        headers = list()
        kwargs["_return_http_data_only"] = True
        kwargs["query"] = {"fromImage": img, "tag": tag}
        parts = img.split("/")
        if parts[0] in iregs:
            auth = iregs[parts[0]].get("auth", None)
            if not auth:
                raise ApiException(
                    "503", f"Authentication not configured for registry {parts[0]}"
                )
            headers.append(f"X-Registry-Auth: {auth}")
        res = self._call(
            "/endpoints/{}/docker/images/create".format(node.id),
            "POST",
            None,
            headers,
            **kwargs,
    ...
    def get_containers(self, node: Node, **kwargs):
        kwargs["_return_http_data_only"] = True
        res = self._call(
            "/endpoints/{}/docker/containers/json".format(node.id),
            "GET",
            None,
            **kwargs,
        )
    ...
    def inspect_container(self, node: Node, cid, **kwargs):
        kwargs["_return_http_data_only"] = True
        res = self._call(
            "/endpoints/{}/docker/containers/{}/json".format(node.id, cid),
            "GET",
            None,
            **kwargs,
        )
    ...
    def create_container(self, node: Node, image, name=None, **kwargs):
        ...
        res = self._call(
            "/endpoints/{}/docker/containers/create".format(node.id),
            "POST",
            body,
            **kwargs,
        )
    ...
    def start_container(self, node: Node, cid, service=None, **kwargs):
        kwargs["_return_http_data_only"] = True
        res = self._call(
            "/endpoints/{}/docker/containers/{}/start".format(node.id, cid),
            "POST",
            None,
            **kwargs,
        )
    ...
    def stop_container(self, node: Node, cid, **kwargs):
        ...
        try:
            res = self._call(
                "/endpoints/{}/docker/containers/{}/stop".format(node.id, cid),
                "POST",
                None,
                **kwargs,
            )
    ...
    def get_networks(self, node: Node, nid=None, **kwargs):
        ...
        if nid:
            res = self._call(
                "/endpoints/{}/docker/networks/{}".format(node.id, nid),
                "GET",
                None,
                **kwargs,
            )
            ...
            res = self._call(
                "/endpoints/{}/docker/networks".format(node.id), "GET", None, **kwargs
            )
        string = res.read().decode("utf-8")
        return json.loads(string)
    ...
    def connect_network(self, node: Node, nid, cid, **kwargs):
        ...
        kwargs["_return_http_data_only"] = True
        res = self._call(
            "/endpoints/{}/docker/networks/{}/connect".format(node.id, nid),
            "POST",
            body,
            **kwargs,
        )
    ...
    def create_network(self, node: Node, name, **kwargs):
        ...
        kwargs["_return_http_data_only"] = True
        self._call(
            "/endpoints/{}/docker/networks/create".format(node.id),
            "POST",
            body,
            **kwargs,
        )
    ...
    # Logs
    def get_logs(
        self, node: Node, cid, since=0, stderr=1, stdout=1, tail=100, timestamps=0
    ):
        kwargs = dict()
        kwargs["_return_http_data_only"] = True
        kwargs["query"] = {
        ...
        res = self._call(
            "/endpoints/{}/docker/containers/{}/logs".format(node.id, cid),
            "GET",
            None,
            **kwargs,
        )
    ...
    def exec_create(self, node: Node, cid, **kwargs):
        ...
        kwargs["_return_http_data_only"] = True
        res = self._call(
            "/endpoints/{}/docker/containers/{}/exec".format(node.id, cid),
            "POST",
            body,
            **kwargs,
        )
    ...
    def exec_start(self, node: Node, ectx, **kwargs):
        ...
        res = self._call(
            "/endpoints/{}/docker/exec/{}/start".format(node.id, eid),
            "POST",
            body,
            **kwargs,
        )
    ...
    def _call(self, url, method, body, headers=[], **kwargs):
        ...
        # HTTP header `Accept`
        header_params["Accept"] = self.client.select_header_accept(["application/json"])  # noqa: E501
        ...
        def _build_kwargs(p):
            ...
            if ipam:
                ...
                for i in ipam.get("config"):
                    docker_kwargs["IPAM"]["Config"].append(
                        {"Subnet": i.get("subnet"), "Gateway": i.get("gateway")}
                    )
            opts = p.settings.options
            if opts:
                docker_kwargs["Options"] = dict()
                if "mtu" in opts and p.settings.driver == "bridge":
                    docker_kwargs["Options"].update(
                        {"com.docker.network.driver.mtu": opts.get("mtu")}
                    )
    ...
    def create_service_record(
        self, sname, sreq: SessionRequest, addrs_v4, addrs_v6, cports, sports
    ):
        srec = dict()
        node = sreq.node
        prof = sreq.profile
        ...
        if cport:
            docker_kwargs["HostConfig"]["PortBindings"].update(
                {"{}/tcp".format(cport): [{"HostPort": "{}".format(cport)}]}
            )
            docker_kwargs["ExposedPorts"].update({"{}/tcp".format(cport): {}})

        if sport:
            docker_kwargs["HostConfig"]["PortBindings"].update(
                {"{}/tcp".format(sport): [{"HostPort": "{}".format(sport)}]}
            )
            docker_kwargs["ExposedPorts"].update({"{}/tcp".format(sport): {}})
        ...
        if expr:
            ...
            for port in exp_ports:
                ...
                docker_kwargs["ExposedPorts"].update({"{}/tcp".format(port): {}})
                ...
                for d in devices:
                    if dnet.name:
                        if "rdma_cm" in d["names"]:
                            dev = {
                                "PathOnHost": os.path.join(d["devprefix"], "rdma_cm"),
                                "PathInContainer": os.path.join(
                                    d["devprefix"], "rdma_cm"
                                ),
                        ...
                        if "uverbs" in d["names"]:
                            ...
                            for iface in vfs:
                                ...
                                dev = {
                                    "PathOnHost": os.path.join(d["devprefix"], n),
