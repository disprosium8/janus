...
from janus.api.models import Node, SessionRequest, AddEndpointRequest
...
class JanusEdgeApi(Service):
    ...
    def _send_message(self, edge: EdgeServerSocket, event, value):
        ...
        if "error" in reply:
            log.error(
                f"received reply from {edge}:event={message['event']}:error={reply['error']}"
            )
    ...
    def resolve_networks(self, node: dict, prof, **kwargs):
        ...
        for net in [Network(prof.settings.mgmt_net), Network(prof.settings.data_net)]:
            if (
                net.name
                and net.name
                not in [Constants.NET_NONE, Constants.NET_HOST, Constants.NET_BRIDGE]
                and not net.is_host()
            ):
    ...
    def exec_stream(self, node: Node, container, eid, **kwargs):
        ...
        if not self._exec_map.get(node_name):
            ...
            self._exec_map[node_name][container] = ExecWebsocketServerSession(
                edge.sock, edge.mutex
            )
        else:
            self._exec_map[node_name][container] = ExecWebsocketServerSession(
                edge.sock, edge.mutex
