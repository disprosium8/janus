...
def send_event(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        cfg.sm.pubsub.publish(
            {
                "msg": {
                    "handler": EPType.SLURM.name,
                    "event": func.__name__,
                    "value": ret,
                }
            },
...
class JanusSlurmApi(Service):
    ...
    def __init__(self):
        ...
        if self.api_url:
            ...
            self._config = c
            self._headers = {
                "X-SLURM-USER-NAME": self.api_user,
    ...
    def get_nodes(self, nname=None, cb=None, refresh=False):
        ...
        archs = set()
        for n in res.nodes:
            cnode = {"name": n.name, "addresses": [n.address]}
    ...
    def start_container(self, node: Node, container: str, service=None, **kwargs):
        ...
        def nodelist(service, job_id):
            while True:
                try:
                    ...
                    res = requests.get(
                        f"{self.api_url}/slurm/{self.DEF_VER}/job/{job_id}",
        ...
        # jid = sub.job_id
        req = {"script": script, "job": kw}
        try:
            res = requests.post(
                f"{self.api_url}/slurm/{self.DEF_VER}/job/submit",
                data=json.dumps(req),
                headers=self._get_headers(service),
            )
    ...
    def stop_container(self, node: Node, container, **kwargs):
        ...
        try:
            res = requests.delete(
                f"{self.api_url}/slurm/{self.DEF_VER}/job/{container}",
                headers=self._get_headers(service),
            )
            if res.status_code != 200:
                raise Exception(f"Slurm API returned {res.status_code}: {res.text}")
    ...
    def create_service_record(
        ...
        if args_override:
            shlex.split(args_override)
        elif prof.settings.arguments:
            shlex.split(prof.settings.arguments)
