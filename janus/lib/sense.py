...
class SENSEMetaManager(DBHandler):
    def __init__(self, cfg: JanusConfig, properties: dict, sense_api_handler=None):
        ...
        self.sense_api_handler = sense_api_handler or SENSEApiHandler(
            url=url, req_wrapper=RequestWrapper()
        )
        self.properties = properties
        self.session_manager = SessionManager()
        self.retries = SenseConstants.SENSE_PLUGIN_RETRIES
        self.counter = 0
        janus_session_creation = properties[
            SenseConstants.SENSE_JANUS_SESSION_CREATION
        ].lower()
        self.create_janus_sessions = (
            SenseConstants.SENSE_JANUS_SESSION_CREATE in janus_session_creation
        )
        self.start_janus_sessions = (
            SenseConstants.SENSE_JANUS_SESSION_START in janus_session_creation
        )
        log.info(
            f"Initialized {__name__}:properties={json.dumps(properties, indent=2)}"
        )
    ...
    def update_metadata(self, agents):
        ...
        ret = self.sense_api_handler.post_metadata(
            metadata=metadata, domain=domain_info[0], name=name
        )

        if ret:
            utc_iso_str = datetime.strftime(
                datetime.now(timezone.utc), "%Y-%m-%dT%H:%M:%S.%f"
            )[:-3]
            name += "_SERVER_INFO"
    ...
    def _instances(targets):
        ...
        for t in targets:
            if "cluster_info" in t:
                instances.append(
                    dict(name=t["cluster_info"]["cluster_name"], nodeName=t["name"])
                )
            else:
    ...
    def create_janus_session(self, sense_session, host_networking=False):
        ...
        for target in targets:
            ...
            if not host_networking:
                ...
                option.update(
                    dict(
                        name=f"{network_profiles[0]}-{vlan}",
                        vlan=str(vlan),
                        parent=f"vlan.{vlan}",
                        mtu=1500,
                    )
                ...
                addr = target["ip"]
                option.update(
                    dict(ip_addr=addr[: addr.index("/")] if "/" in addr else addr)
                )
        ...
        log.info(
            f"creating janus sample session using sense_session {sense_session['name']}:{requests}"
        )
    ...
    def terminate_janus_sessions(self, sense_session: dict):
        for janus_session in self.find_janus_session(
        ...
        for v in sense_session.get("networks", dict()).values():
            ...
            if not cluster:
                log.warning(
                    f"did not find cluster {cluster_name} while terminating sessions {sense_session}"
                )
                continue
    ...
    def _retrieve_instance_termination_notice_command(self, task):
        ...
        if not sense_session:
            self.sense_api_handler.finish_task(
                task_id, list(), f"no session found for {instance_id}:{alias}"
            )
            return None
    ...
    def _retrieve_handle_sense_instance_command(
        self, task, agents=None, node_names=None
    ):
        ...
        unknown_endpoints = [
            endpoint for endpoint in endpoints if endpoint not in node_names
        ]
        if unknown_endpoints:
            log.warning(
                f"unknown endpoint for instance {instance_id}:endpoints={unknown_endpoints}"
            )
            self.sense_api_handler.reject_task(
                task_id, targets, f"unkown targets:{unknown_endpoints}"
            )
            return None
        ...
        sense_session["state"] = (
            "MODIFIED" if self.check_modified(sense_session) else "OK"
        )
        self.save_sense_session(sense_session=sense_session)
    ...
    def retrieve_tasks(self, agents=None):
        ...
        tasks = self.sense_api_handler.retrieve_tasks(
            assigned=assigned, status="PENDING"
        )
        ...
        for task in tasks:
            ...
            try:
                ...
                task_id = task["uuid"]
                log.debug(
                    f"RETRIEVED_TASK:command={command}:task_id={task_id}:targets={targets}"
                )

                if command not in [
                    "handle-sense-instance",
                    "instance-termination-notice",
                ]:
                    self.sense_api_handler.reject_task(
                        task_id, targets, f"unknown command:{command}"
                    )
                ...
                if (
                    sense_session
                    and "status" in sense_session
                    and sense_session["status"] == "PENDING"
                ):
                    log.debug(
                ...
                log.debug(
                    f"HANDLING_RETRIEVED_TASK:command={command}:task_id={task_id}:targets={targets}"
                )
                users = list()
                ...
                if not targets or not users or command == "instance-termination-notice":
                    ...
                    ret = self._retrieve_handle_sense_instance_command(
                        task, agents, node_names
                    )
                ...
                if task_id:
                    self.sense_api_handler.fail_task(
                        task_id, targets, f"retrieve_task:{type(e)}:{e}"
                    )
    ...
    def _finish_handle_sense_instance_command(self, task_id, sense_session):
        ...
        if modified:
            ...
            try:
                ...
                sense_session["terminate_error_message"] = (
                    f"error terminating janus session: {e}"
                )
                error = True
                ...
                try:
                    if sum(sense_session["task_info"].values(), []):
                        ...
                        if self.create_janus_sessions:
                            janus_session_ids = self.create_janus_session(
                                sense_session=sense_session
                            )
                            sense_session["janus_session_id"] = janus_session_ids

                            if self.start_janus_sessions:
                                self.session_manager.start_session(
                                    session_id=janus_session_ids[0]
                                )
                    else:
                    ...
                    sense_session["errors"] += 1
                    sense_session["error_message"] = (
                        f"error creating profiles or janus session: {e}"
                    )
        ...
        if 0 < sense_session["errors"] < self.retries:
            ...
            mesg = (
                sense_session["terminate_error_message"]
                if sense_session["terminate_error_message"]
                else sense_session["error_message"]
            )
        ...
        for sense_session in sense_sessions:
            ...
            try:
                if sense_session["command"] != "handle-sense-instance":
                    ...
                    self._finish_instance_termination_notice_command(
                        task_id, sense_session
                    )
                else:
                ...
                targets = SenseUtils.to_targets(sense_session)
                self.sense_api_handler.fail_task(
                    task_id, targets, f"finish_task:{type(e)}:{e}"
                )
    ...
    def run(self):
        ...
        sense_sessions, rejected_tasks, failed_tasks = self.retrieve_tasks(
            agents=agents
        )
        counters = dict(
            retrieved=0, rejected=0, accepted=0, terminated=0, finished=0, failed=0
        )
        print_summary = False
        ...
        if sense_sessions:
            ...
            counters["finished"] = len(
                [s for s in sense_sessions if s["status"] == "FINISHED"]
            )
            counters["terminated"] = len(
                [s for s in sense_sessions if s["status"] == "DELETED"]
            )
            counters["failed"] = len(
                [s for s in sense_sessions if s["status"] == "FAILED"]
            )
            print_summary = True
        ...
        if print_summary:
            ...
            sense_session_summaries = [
                to_session_summary(
                    s, self.find_janus_session(host_profile_names=s["host_profile"])
                )
            ...
            server_info = dict(
                counters=counters,
                last_updates=sense_session_summaries,
