...
class DBHandler(object):
    ...
    def save_sense_session(self, sense_session):
        self.db.upsert(
            self.sense_session_table, sense_session, "key", sense_session["key"]
        )

    def find_sense_session(
        self, *, user=None, sense_session_key=None, name=None, status=None
    ):
        queries = list()
        ...
        sense_sessions = self.db.search(
            self.sense_session_table, query=reduce(lambda a, b: a & b, queries)
        )
        return sense_sessions

    def check_modified(self, new_sense_session):
        sense_sessions = self.find_sense_session(
            sense_session_key=new_sense_session["key"]
        )
        ...
        for target in old_targets:
            ...
            parent = (
                parent if parent and "?" not in parent else f"vlan.{target['vlan']}"
            )
    ...
    def save_network_profile(self, network_profile):
        self.db.upsert(
            self.network_table, network_profile, "name", network_profile["name"]
        )
    ...
    def find_network_profiles(self, *, name):
        network_profiles = self.db.search(
            self.network_table, query=(Query().name == name)
        )
    ...
    def find_host_profiles(self, *, user=None, name=None, net_name=None):
        ...
        host_profiles = self.db.search(
            self.host_table, query=reduce(lambda a, b: a & b, queries)
        )
    ...
    def find_janus_session(self, *, user=None, name=None, host_profile_names=None):
        ...
        if host_profile_names:
            queries.append(
                Query().request.any(Query().profile.one_of(host_profile_names))
            )

        if not queries:
            return list()

        janus_sessions = self.db.search(
            self.janus_session_table, query=reduce(lambda a, b: a & b, queries)
        )
    ...
    def get_or_create_network_profile(
        self, name, target, users, subnet, groups=None, host_networking=False
    ):
        ...
        if host_networking:
            ...
            network_profile = dict(
                name=name + "-host",
                settings=network_profile_settings,
                users=users,
                groups=groups,
            )
        else:
            ...
            config.append(
                dict(
                    subnet=subnet[0 : idx + 1] + "0/" + str(prefixlen),
                    gateway=subnet[0 : idx + 1] + "1",
                )
            )
            network_profile_settings = {
                "driver": "macvlan",
    ...
    def get_or_create_host_profile(
        self, name, network_profile, addr, users, groups=None, host_networking=False
    ):
        if host_networking:
            host_profile_settings = dict(
                cpu=0.2,
                memory=1073741824,
                mgmt_net=dict(
                    name=network_profile["name"], ipv4_addr=addr, ipv6_addr=None
                ),
        ...
        default_settings.update(
            (k, host_profile_settings[k])
            for k in default_settings.keys() & host_profile_settings.keys()
        )

        groups = groups or list()
        host_profile = dict(
            name=name, settings=default_settings, users=users, groups=groups
        )
    ...
    def create_profiles(self, sense_session, host_networking=False):
        ...
        nprofs = self.get_or_create_network_profile(
            name=name + "-net",
            target=targets[0],
            subnet=subnets[0],
            users=users,
        ...
        assert len(nprofs) == 1
        addr = None
        hprof = self.get_or_create_host_profile(
            name=name,
            network_profile=nprofs[0],
            addr=addr,
            users=users,
            host_networking=host_networking,
        )
    ...
    def update_janus_sessions(self, sense_session: dict):
        ...
        for janus_session in self.find_janus_session(
            ...
            janus_session["user"] = (
                sense_session["users"][0] if sense_session["users"] else "admin"
            )
            self.db.update(
                self.janus_session_table, janus_session, ids=janus_session["id"]
            )
