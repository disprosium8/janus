...
class SenseConstants:
    ...
    SENSE_JANUS_SESSION_CREATION_DEFAULTS = (
        f"{SENSE_JANUS_SESSION_CREATE},{SENSE_JANUS_SESSION_START}"
    )
...
class SenseUtils:
    ...
    def parse_from_config(
        cfg: JanusConfig, parser: ConfigParser, plugin_section="PLUGINS"
    ):
        if plugin_section in parser:
            for plugin in parser[plugin_section]:
                if plugin == "sense-metadata-plugin":
                    sense_meta_plugin = parser.get(
                        "PLUGINS", "sense-metadata-plugin", fallback=None
                    )

                    if sense_meta_plugin:
                        sense_properties = dict()
                        cfg.sense_metadata = parser.getboolean(
                            sense_meta_plugin, "sense-metadata-enabled", fallback=False
                        )
                        sense_metadata_url = parser.get(
                            sense_meta_plugin,
                            SenseConstants.SENSE_METADATA_URL,
                            fallback=None,
                        )
                        sense_metadata_assigned = parser.get(
                            sense_meta_plugin,
                            SenseConstants.SENSE_METADATA_ASSIGNED,
                            fallback=SenseConstants.JANUS_DEVICE_MANAGER,
                        )
                        sense_metadata_domain_info = parser.get(
                            sense_meta_plugin,
                            SenseConstants.SENSE_DOMAIN_INFO,
                            fallback="JANUS/AES_TESTING",
                        )

                        sense_properties[SenseConstants.SENSE_METADATA_URL] = (
                            sense_metadata_url
                        )
                        sense_properties[SenseConstants.SENSE_DOMAIN_INFO] = (
                            sense_metadata_domain_info
                        )
                        sense_properties[SenseConstants.SENSE_METADATA_ASSIGNED] = (
                            sense_metadata_assigned
                        )
                        sense_janus_creation = parser.get(
                            sense_meta_plugin,
    ...
    def to_target_summary(targets: list):
        ...
        for target in targets:
            if "cluster_info" in target:
                ret.append(
                    dict(name=target["name"], cluster_info=target["cluster_info"])
                )
            else:
    ...
    def to_targets(sense_session):
        ...
        if network_profiles := sense_session.get("network_profile", list()):
            for idx, target in enumerate(targets):
                network_profile = (
                    network_profiles[idx]
                    if idx < len(network_profiles)
                    else network_profiles[0]
                )
        ...
        if host_profiles := sense_session.get("host_profile", list()):
            for idx, target in enumerate(targets):
                host_profile = (
                    host_profiles[idx] if idx < len(host_profiles) else host_profiles[0]
                )
        ...
        if janus_sessions := sense_session.get("janus_session_id", list()):
            for idx, target in enumerate(targets):
                janus_session = (
                    janus_sessions[idx]
                    if idx < len(janus_sessions)
                    else janus_sessions[0]
                )
    ...
    def to_sense_session_summary(sense_session: dict, janus_sessions):
        ...
        return dict(
            sense_session=sense_session["name"],
    ...
    def dump_sessions_requests(session_requests):
        ...
        for sr in session_requests:
            ...
            profile = dict(
                name=dumpable_sr["profile"]["name"],
                settings=dict(data_net=dumpable_sr["profile"]["settings"]["data_net"]),
            )
