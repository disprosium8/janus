            ...
            "test-caltech-host-networking": [
                {
                    ...
                    "vlan": None,
                    "ip": None,
                    "cluster_info": {"cluster_name": "kubernetes-admin@kubernetes"},
                },
                {
...
def run_host_networking_task_workflow(sense_session):
    ...
    print("SrcServiceInfo:", json.dumps(service_info[0], indent=2))
    ...
    print("ExecRequest:", json.dumps(req, indent=2))
    ...
    res = handler.exec_stream(
        node=Node(**node), container=service_info[0]["sname"], eid=None
    )
...
def test_caltech_host_networking():
    ...
    try:
        ...
        if "list index out of range" in str(e) or "Invalid kube-config" in str(e):
            pytest.skip(
                f"Kubernetes environment not available or context mismatch: {e}"
            )
        raise e
